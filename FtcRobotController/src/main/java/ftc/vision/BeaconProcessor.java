 ...
 * FTC Team EV 7393
 */
public class BeaconProcessor implements ImageProcessor<BallColourResults> {
    private static final String TAG = "BeaconProcessor";
    private static final double MIN_MASS = 6;

    @Override
    public ImageProcessorResult<BallColourResults> process(long startTime, Mat rgbaFrame, boolean saveImages) {
        ...
        BallColourResults.BallColour[] BallColours = BallColourResults.BallColour.values();
        BallColourResults.BallColour left = BallColours[maxMassIndex[0]];
        BallColourResults.BallColour right = BallColours[maxMassIndex[1]];

        //draw the color result bars
        int barHeight = hsv.height()/30;
        Imgproc.rectangle(rgbaFrame, new Point(0, 0), new Point(hsv.width()/2, barHeight), left.colour, barHeight);
        Imgproc.rectangle(rgbaFrame, new Point(hsv.width()/2, 0), new Point(hsv.width(), barHeight), right.colour, barHeight);
        ...
        return new ImageProcessorResult<>(startTime, rgbaFrame,
                new BallColourResults(left, right)
        );
    }
}
